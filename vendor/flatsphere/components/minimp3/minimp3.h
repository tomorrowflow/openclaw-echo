...
/* ESP-IDF: use PSRAM for large scratch buffer */
#ifdef ESP_PLATFORM
#include "esp_heap_caps.h"
#define MALLOC_CAP_SPIRAM_DEFINED
#endif
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    const uint8_t *hdr;
    bs_t bs_frame[1];
    /* scratch is ~16KB — heap-allocate to avoid stack overflow on ESP32 */
    static mp3dec_scratch_t *scratch_ptr = NULL;
    if (!scratch_ptr) {
#ifdef ESP_PLATFORM
        scratch_ptr = (mp3dec_scratch_t *)heap_caps_malloc(sizeof(mp3dec_scratch_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
        scratch_ptr = (mp3dec_scratch_t *)malloc(sizeof(mp3dec_scratch_t));
#endif
        if (!scratch_ptr) {
            info->frame_bytes = 0;
            return 0;
